...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
		...
		void Update()
		{
			...
			{
				if (currentControllerState is ControllerState.Jumping or ControllerState.Falling or ControllerState.Rising)
				{
					mover.SetVelocity(Vector3.zero);
					momentum = new Vector3(momentum.x, doubleJumpForce, momentum.z);
					OnJumpStart();
